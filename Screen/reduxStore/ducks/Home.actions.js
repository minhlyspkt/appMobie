import { createAction, createAsyncThunk } from '@reduxjs/toolkit'
import { Alert } from 'react-native';
import HomeModel from "../../Model/HomeModel";
export const updateState = createAsyncThunk('Home/updateState', async (payload) => {
  let dataModel = await updateStateAsync(payload)
  console.log(dataModel)
  return dataModel
})

export const updateName = createAction('Home/updateName')

export const getHomePageData = createAsyncThunk('Home/getHomePageData', async (payload) => {
  let dataModel = await getHomeData()
  return dataModel
})

export const submitUpdate = createAsyncThunk('Home/submitUpdate', async (payload) => {
  let dataModel = await HomeModel.UpdateNameStorage(payload)
  console.log(dataModel)
  return dataModel
})

export const getPublicIP = createAsyncThunk('Home/getPublicIP', async (payload) => {
  let dataModel = await getPublicIPAsync(payload)
  console.log(dataModel)
  return dataModel
})

export const submitPublicIP = createAsyncThunk('Home/submitPublicIP', async (payload) => {
  let dataModel = await submitPublicIPAsync(payload)
  console.log(dataModel)
  return dataModel
})

getHomeData = async () => {
  let dataModel = await HomeModel.GetDataHome("")
  // var myHeaders = new Headers()
  // myHeaders.append("Authorization", "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IkhCeGw5bUFlNmd4YXZDa2NvT1UyVEhzRE5hMCIsImtpZCI6IkhCeGw5bUFlNmd4YXZDa2NvT1UyVEhzRE5hMCJ9..TALL1k7AScj0OtiyoAoNE0ZIdT4ylyvDDCU-0EnehThG_lqJaoBRepupRGKkzaSo5P61B8tPdXEpXVr3ulsNCOwKMfhI1yWiDK-EPO0UH0Cx86j3cAf4LEA9jeka_2mpBIBLZda92w0mFzOTj4QXizxFpYe6Jga7kehqzF3HLuOtPsIpouat6PsJG-GwzFnZrasp60Z6QqDwkSSiY-qe4JLqZlUZuAljY2jJLH-316RiLkPWb3e6yP7hMu9mrnRq0sQ8UcYadfMprqZ0eMNrUQu3poqVzP1NwomELsRKrUJJ7bTXdzS21azF3XTAMP-UlNQEVNwWhNnDKa6tFwoRjw");
  // myHeaders.append("Content-Type", "application/json")

  // var raw = JSON.stringify({ "Tocken": "eyJ0eXAiOiJKV1QiLCJhbGciOihVUyAtIFByaW" })
  // //var url = HomeModel.GetUrl(this.router)
  // var requestOptions = {
  //   method: 'POST',
  //   headers: myHeaders,
  //   body: raw,
  //   redirect: 'follow'
  // };

  // await fetch("http://192.168.1.184:147/131220-2-4-33-L3/output", requestOptions)
  //   .then(response => response.text())
  //   .then(async (result) => {
  //     let response = JSON.parse(result)
  //     if (response.Status) {
  //       dataModel = await HomeModel.GetDataHome(response.data)
  //       // let dataTest = []
  //       // for (let i = 0; i < 10; i++) {
  //       //   dataTest[i] = { key: i, Status: "0", Name: "Device " + (i), GPIO: i }
  //       // }
  //       // this.setState({ data: dataModel, isloading: true, loading: false })
  //     }
  //   }
  //   )
  //   .catch(error => console.log('error', error))
  return dataModel
}

updateStateAsync = async (data) => {
  let responseData = null
  var myHeaders = new Headers();
  myHeaders.append("Authorization", "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IkhCeGw5bUFlNmd4YXZDa2NvT1UyVEhzRE5hMCIsImtpZCI6IkhCeGw5bUFlNmd4YXZDa2NvT1UyVEhzRE5hMCJ9..TALL1k7AScj0OtiyoAoNE0ZIdT4ylyvDDCU-0EnehThG_lqJaoBRepupRGKkzaSo5P61B8tPdXEpXVr3ulsNCOwKMfhI1yWiDK-EPO0UH0Cx86j3cAf4LEA9jeka_2mpBIBLZda92w0mFzOTj4QXizxFpYe6Jga7kehqzF3HLuOtPsIpouat6PsJG-GwzFnZrasp60Z6QqDwkSSiY-qe4JLqZlUZuAljY2jJLH-316RiLkPWb3e6yP7hMu9mrnRq0sQ8UcYadfMprqZ0eMNrUQu3poqVzP1NwomELsRKrUJJ7bTXdzS21azF3XTAMP-UlNQEVNwWhNnDKa6tFwoRjw");
  myHeaders.append("Content-Type", "application/json");

  var raw = JSON.stringify({ "Tocken": "eyJ0eXAiOiJKV1QiLCJhbGciOihVUyAtIFByaW", "Status": data.Status, "GPIO": data.GPIO });

  var requestOptions = {
    method: 'POST',
    headers: myHeaders,
    body: raw,
    redirect: 'follow'
  };

  await fetch("http://192.168.1.184:147/131220-2-4-33-L3", requestOptions)
    .then(response => response.text())
    .then(result => {
      let response = JSON.parse(result)
      if (response.Status) {
        responseData = data
      }
    }
    )
    .catch(error => console.log('error', error));
  return responseData
}

getPublicIPAsync = async () => {
  let valueStorage = await HomeModel.getPublicIPStorage()
  return valueStorage
}

submitPublicIPAsync = async () => {
  let responseData = null
  let valueStorage = await HomeModel.getConnectStorage()
  if (valueStorage == "1") {
    Alert.alert('Please login with option "Private"')
    return responseData
  }
  var requestOptions = {
    method: 'GET',
    redirect: 'follow'
  };

  await fetch("https://api.ipify.org", requestOptions)
    .then(response => response.text())
    .then(async (result) => 
      {
        await HomeModel.updatePublicIPStorage(result)
        responseData = result
      }
      )
    .catch(error => console.log('error', error));
  return responseData
}
export default {
  getHomePageData,
  updateState,
  updateName,
  getPublicIP,
  submitPublicIP
}
